...
//    		"06"	//06 local
    ...
    public static void main(String[] args) {
		...
		//初始化batch
		if(isBatch) {
			J71_Tran_Util.scanBatch(CP,batchSet);//扫描代码根目录，获得所有flowtrans,并入库
			J3_Util.logE("136-init batch");
			J3_Util.print(batchSet);
	    	if(!isRealTime) {
...
//			J3_Util.printDebug(rootTree);
			if(isPrint) {
				try {
//					J3_Util.pringToJson(J3_Util.getBranchSb(rootTree), ((Map)J2_Config.CONFIG.get(PROJECT_ID)).get(J2_Main.JSON_PATH).toString());
					J3_Util.logE("1392-print menu");
				} catch (Exception e) {
					e.printStackTrace();
				} 
			}
